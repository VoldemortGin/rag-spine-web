...
<Cards>
  ...
  <Card title="Everything is a Protocol">
    LLM provider, embeddings, reranker, OCR, retriever, graph store, and task queue are all
    typed `Protocol`s injected at the edges. The core imports zero SDKs and runs offline.
  </Card>
</Cards>
...
<TypeTable
  type={{
    ...
    GraphStore: {
      type: 'Protocol',
      description:
        'graph/store.py — upsert_nodes/edges, neighbors, subgraph, traverse. Zero-dep in-process deterministic default; networkx adapter behind [graph]. Backs the opt-in structured relation graph.',
    },
    OcrBackend: {
      type: 'Protocol',
