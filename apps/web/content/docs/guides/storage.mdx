...
<Files>
  <Folder name="storage" defaultOpen>
    <File name="fact_store.py — Fact dataclass + FactStore Protocol + SqliteFactStore (table fact_metric)" />
  </Folder>
...
<TypeTable
  type={{
    ...
    corrected_by: {
      type: 'str | None',
      default: 'None',
      description: 'Correction lineage — the SME who applied the correction.',
    },
    corrected_audit_seq: {
      type: 'int | None',
      default: 'None',
      description: 'Correction lineage — the review-audit seq behind the value.',
    },
    dimensions: {
      type: 'dict[str, str]',
      default: '{}',
...
`FactStore` is a `@runtime_checkable` **Protocol** — the seam the core imports (use it only as
a type hint / `isinstance` target; instantiating it raises). The class you construct is the
zero-dependency sqlite default, `SqliteFactStore`, either directly or through the
`make_fact_store(spec, **kwargs)` factory / `RAGSPINE_FACT_STORE` env var (third-party
backends register under the `ragspine.fact_stores` entry-point group).
...
| `set_review_status(dim_key, status) -> int`                                                                               | human-review write-back: flip one fact's `review_status` by `dim_key`; returns 0 or 1                                  |
| `dim_key_for(fact) -> str` _(static)_                                                                                     | compute a `Fact`'s `dim_key` from its typed identity columns (the public accessor — `dim_key` is never a `Fact` field) |
| `get_by_dim_key(dim_key) -> Fact \| None`                                                                                 | fetch one fact by `dim_key` (any review status); `None` if absent                                                      |
...
[anti-fabrication invariant](/en/docs/concepts/anti-fabrication) relies on — no `found` fact
...
valid_as_of, ingested_at, corrected_by, corrected_audit_seq`) — and stamps `ingested_at` itself. Re-ingesting the same data
therefore never grows the store; this is the backbone of
[idempotent ingestion](/en/docs/guides/ingestion#the-batch-manifest-ledger).
...
`version`, and `active`. Current schemas add `parent_id`, `heading`, `window_text`, and
`parent_locator`. `init_schema()` adds those columns with empty defaults to an older database, so
existing chunk stores remain readable.

`window_text` is generation-only expansion context. A retrieval hit and citation retain the child
`text`, `chunk_id`, and `source_locator`; `parent_locator` is a provenance backreference, not a
replacement locator. Restricted children are removed with their expansion context before prompt
assembly.
...
<Callout type="info">
  ...
  `sensitivity` column is what powers [RESTRICTED isolation](/en/docs/concepts/restricted-isolation)
  downstream.
</Callout>
...
Defaults come from [`common/core.py`](/en/docs/guides/common#core--global-constants):
...
<Cards>
  ...
  <Card title="Extraction guide — the Fact source" href="/en/docs/guides/extraction" />
  <Card title="Concept: Anti-fabrication" href="/en/docs/concepts/anti-fabrication" />
