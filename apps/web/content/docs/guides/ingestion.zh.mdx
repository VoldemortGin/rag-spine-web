...
写入[存储](/docs/guides/storage)。它有三条车道：**结构化**（数值
...
- `ingest_file(path, store, registry, queue, *, dry_run=False, manifest=None, batch_id=None, valid_as_of=None, grid_extractor=None)` —
  ...
  xlsm / pptx，或经[路由器](/docs/guides/extraction#pdf-路由--逐页分诊)处理的 PDF），
  并复用共享的摄取逻辑。
...
[`Fact`](/docs/guides/storage#fact-dataclass) 对象，再用
...
<Callout type="warn">
  **幂等性真正的所在。** 契约把 manifest 称作"守卫"，它确实是每次
  ...
  [`dim_key`](/docs/guides/storage#dim_key--upsert-键) 为键）：重跑一个批次会重新抽取并
...
[ChunkStore](/docs/guides/storage#chunk-存储)。
...
<Callout type="info">
  ...
  来自 [common](/docs/guides/common#敏感度) 的 `classify_sensitivity(...)`。这正是
...
[术语表中的评审队列](/docs/concepts/glossary#审核队列)。
...
内置名胜出）。参见[扩展点 → SourceConnector](/docs/reference/extension-points#sourceconnector)。
...
<Cards>
  <Card title="抽取指南" href="/docs/guides/extraction" />
  <Card title="存储指南" href="/docs/guides/storage" />
  <Card title="Common 指南——敏感度与词表" href="/docs/guides/common" />
  <Card title="术语表中的评审队列条目" href="/docs/concepts/glossary#审核队列" />
</Cards>
