...
[防编造机制不变量](/docs/concepts/anti-fabrication)所依赖的——没有 `found` 事实
就意味着编排器会把答案重写为"未找到"。
...
绝不会让存储膨胀；这是
[幂等摄取](/docs/guides/ingestion#批次-manifest-台账)的支柱。
...
<Callout type="info">
  ...
  `sensitivity` 列正是下游 [RESTRICTED 隔离](/docs/concepts/restricted-isolation)
  的动力来源。
</Callout>
...
默认值来自 [`common/core.py`](/docs/guides/common#core--全局常量)：
...
<Cards>
  ...
  <Card title="抽取指南 — Fact 的来源" href="/docs/guides/extraction" />
  <Card title="概念：防编造机制" href="/docs/concepts/anti-fabrication" />
