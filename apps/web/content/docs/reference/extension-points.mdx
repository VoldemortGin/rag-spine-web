...
<Callout type="warn">
  ...
  lineage from the chunk (the caller) rather than the model's self-report, and drops any edge
  whose endpoint the deterministic `SecurityGate` refuses. A model-asserted relation can never
  read as a controlled, verified fact.
</Callout>
