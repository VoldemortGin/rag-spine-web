---
title: 'ADR 0009: "Framework-free" redefined — no framework lock-in + permissive-license-only'
description: Redefine framework-free as no orchestration lock-in plus permissive-license-only; real backends become opt-in extras for install weight, not purity.
---
...
- **Absolute zero-SDK core, all real backends in extras for purity (option A)**:
  rejected as the _rationale_ — torch/numpy are licensed-and-lock-in acceptable, so
...
- Relaxes [0005](/en/docs/decisions/adr-0005)/[0006](/en/docs/decisions/adr-0006):
