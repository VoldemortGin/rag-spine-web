---
title: 'ADR 0010: Intent parsing — deterministic security gate + pluggable IntentParser'
description: Decouple the always-on, never-pluggable deterministic security gate from a pluggable IntentParser Protocol — deterministic where it matters, flexible where it's safe.
---
...
Part of the [0002](/en/docs/decisions/adr-0002) product direction. Forced by [0004](/en/docs/decisions/adr-0004) and [0007](/en/docs/decisions/adr-0007).
...
happens _before_ any LLM or retrieval call, and external-entity masking lives here. Two
prior decisions break a pure rule parser: full generality ([0004](/en/docs/decisions/adr-0004))
means arbitrary user-defined dimensions a finance-tuned parser can't read, and
multilingual ([0007](/en/docs/decisions/adr-0007)) makes
...
- **Intent extraction** becomes an **`IntentParser` Protocol** with a default zero-LLM,
  ...
  [0009](/en/docs/decisions/adr-0009)) and an optional LLM-classifier
