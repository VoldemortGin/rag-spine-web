---
title: 'ADR 0011: Adopt the python-project-standard; migrate to src/ layout, keep the rest'
description: Adopt the house standard as governing philosophy; align the one bounded structural invariant (src/ragspine/ layout) and accept four documented divergences.
---
...
and [0005](/en/docs/decisions/adr-0005) (lean core).
...
invariant, and the one whose migration cost is _bounded_: every in-tree import already uses
...
3. SDK isolation via `Protocol` + lazy import (per [0009](/en/docs/decisions/adr-0009) / [0005](/en/docs/decisions/adr-0005)), not a `ports/` +
