...
八条缝 + errors 缝均已落地,各带 Protocol + 离线确定性默认 + 工厂 + 参数化 conformance;
`make ci` 全绿(ruff + mypy + pytest)、import-clean、核心 `dependencies` 为空。当前版本 **0.2.0**。

| 缝 | 公开面 | 离线默认 | 状态 |
|---|---|---|---|
| seam/registry | `Registry` / `lazy_extra_import` | 内置注册 + entry-point 发现 | ✅ |
| observability/trace | `TraceSink` / `TraceExporter` / `TraceEvent` / `TraceError` / `FORBIDDEN_KEYS` | `InProcessPrivacyTraceSink`(拒正文)/ `InProcessTraceExporter`(扇出,0.2.0) | ✅ |
| llm/provider | `LLMProvider.chat`(OpenAI chat-completions 规范:messages+tools 进、`ChatCompletion` 出)/ `StreamingLLMProvider.stream_chat`(叠加协议,0.2.0) | `MockProvider`(确定性,OpenAI 形状;同时满足流式) | ✅ |
| llm/rate_limit | `RateLimitedProvider`(包装任意 LLMProvider,每分钟 token 上限,超限阻塞等待) | 纯标准库滑动窗口(threading + deque,事后按 usage 累计) | ✅ |
| blob/store | `BlobStore` / `make_blob_store` / `BLOB_REGISTRY` / `BlobError` / `BlobNotFound`(0.2.0) | `MemoryBlobStore`(进程内 dict)/ `FileSystemBlobStore`(key 经 sha256 摊平) | ✅ |
...
> **0.2.0 变更(0.1.1 → 0.2.0):**
> - **新增 `blob/store` 缝(ADR 0003,「需求先行的上提」):** spineagent 的 skills/artifacts 与
>   ragspine 的 ingestion 附件本轮同时立项,都需同一块「key → bytes」稳定面。缝的形状取家族已验证的
>   元模式(Protocol + 离线默认 + Registry + conformance),纯字节存储零领域泄漏、两个默认实现纯标准库
>   (dict / pathlib / hashlib),真实后端(S3/MinIO)经可选 extra 延迟 import,`dependencies` 仍为空。
> - **两条缝最小扩 Protocol(守「Protocol 可小步扩,实现走 extra」):** `observability/trace` 加
>   `TraceExporter` 扇出协议 + `InProcessTraceExporter`——扇出严格发生在隐私校验之后,导出面与本地面
>   等宽,隐私不变量不因扇出放松;`llm/provider` 加独立叠加协议 `StreamingLLMProvider`(不给 `LLMProvider`
>   直接加方法,避免破坏 `@runtime_checkable` 兼容),`MockProvider` 给确定性流式默认,流式拼接逐字等于
>   非流式。真实导出 / 流式后端仍走 extra。
...
| # | 项 | 触发条件 | 落点 | 状态 |
|---|---|---|---|---|
| B1 | trace 真实 sink(OTel 等) | ≥2 app 需要导出 trace | 扇出协议 `TraceExporter`(+ `InProcessTraceExporter`)已于 0.2.0 落地核心;**真实导出后端仍走可选 extra / contrib,不进核心默认路径** | 协议 ✅ / 真实后端待证据 |
| B2 | llm streaming / 批量 / token 计量 | ≥2 app 在缝上重复同一形状 | 先扩 Protocol(最小),实现走 extra | streaming Protocol `StreamingLLMProvider`(+ Mock 流式默认)已于 0.2.0 落地 ✅ / 批量 · token 计量待证据 |
