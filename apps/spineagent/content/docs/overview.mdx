...
协议缝 + Sandbox / Skills / Middleware / Artifact 扩展缝 + 统一 LLM provider 适配层(含流式)」
收成一组**小而稳定的原语**。它依赖薄核 `corespine`,复用其缝元模式与 observability / config 形状,
但**不**含任何 RAG 概念。
...
另有**预置** agent `DeepResearchAgent`:**纯组合**已有原语(拆解子查询 → `Coordinator` 并行检索 →
`LlmAgent` 综合),不引入新机制,离线默认零网络即可端到端跑。`AgentResult` 现还带 `artifacts`
(产物引用,见下「artifact 缝」;默认路径不产出)。
...
### 扩展缝:Sandbox / Skills / Middleware / Artifact

四条同元模式的新缝(`Protocol + 离线确定性默认 + Registry 工厂 + entry-point 发现`,组名统一 `corespine.<seam>`):

- **Sandbox**(`spineagent.sandbox.seam`):代码执行隔离缝。`Sandbox` 协议 + `InProcessSandbox`(**白名单 AST**
  纯表达式求值器,只认极小内建白名单,拒 `import` / 属性 / 任意调用;失败归一为非零 `SandboxResult` 不抛)。
  `subprocess` / `container` 是**桩**(`make` 即抛 `SeamError`,`container` 先延迟 import `docker`)。Registry `sandboxes`。
- **Skills**(`spineagent.skills.*`):把 **manifest 文件夹包**(`manifest.toml` + 脚本,默认 `skill.py`)当技能。
  `Skill` 协议 + `FixtureSkill`(经 Sandbox 跑脚本)+ `SkillBundle.load(path)`。`skill_as_function_tool(skill)`
  把技能桥成 `FunctionTool` 供 `FunctionCallingAgent` 用。Registry `skill_registry`。
- **Middleware**(`spineagent.agent.middleware`):围绕任意 agent 一次 `step` 的**洋葱环切面**。`Middleware` 协议
  只有 `before_step` / `after_step` 两钩子;`MiddlewareAgent` 正序进、逆序出地包一圈。四件套:
  `TokenUsageMiddleware` / `SummaryMiddleware` / `DynamicToolMiddleware` / `AttachmentMiddleware`(皆只记元数据,隐私安全)。
  Registry `middlewares`。
- **Artifact**(`spineagent.agent.artifact`):agent 产物落地缝。`Artifact`(带字节)/ `ArtifactRef`(轻量引用,即
  `AgentResult.artifacts` 元素)/ `ArtifactSink` 协议 + `InProcessArtifactSink`。`BlobArtifactSink` **组合 corespine 的
  `BlobStore`**:内容寻址 `sha256` key,`store.put/get` 只管字节往返、元数据全放 `ArtifactRef`(方向 `spineagent → corespine`)。
  Registry `artifact_sinks`。
...
**流式**是可选加成:`OpenAICompatProvider` / `AnthropicProvider` 额外实现 corespine 的 `StreamingLLMProvider`
协议(`stream_chat(...) -> Iterator[ChatCompletionChunk]`,按序拼接 `delta.content` 恒等于非流式结果);
`Cohere` / `Gemini` / `Bedrock` 诚实不实现,`isinstance(..., StreamingLLMProvider)` 如实回 `False`。
