...
// The "core is feature-complete" snapshot, drawn from README — no invented claims.
const STATUS: readonly { area: string; detail: string }[] = [
  {
    area: 'Read',
    detail: 'Parse, malformed-PDF repair, and decrypt (RC4 / AES-128 / AES-256, R2–R6).',
  },
  {
    area: 'Text & tables',
    detail: 'get_text formats, search_for, and find_tables with merged-cell detection.',
  },
  {
    area: 'Edit & save',
    detail: 'Byte-exact incremental save, page ops, insert_pdf merge / split, encryption.',
  },
  {
    area: 'Render & OCR',
    detail: 'get_pixmap rasterizer plus Tesseract and a pure-Rust PaddleOCR engine.',
  },
];
    ...
    <main className="flex flex-1 flex-col">
        ...
        <div className="relative mx-auto w-full max-w-6xl px-6 py-20 sm:py-28">
          <div className="flex flex-col items-center gap-7 text-center">
            ...
            <p className="max-w-2xl text-pretty text-base leading-relaxed text-fd-muted-foreground sm:text-lg">
              pdfspine is a drop-in-shaped, permissively-licensed (Apache-2.0) alternative to AGPL
              PyMuPDF — pure Rust, no C blob, with PyO3 Python bindings and an opt-in{' '}
              <span className="font-mono">import fitz</span> shim. License cleanliness is
              CI-enforced via cargo-deny, not a promise.
            </p>
      ...
      <section className="mx-auto w-full max-w-6xl px-6 py-20 sm:py-24">
        <div className="mb-10 flex flex-col gap-3">
          <SectionLabel>Why pdfspine</SectionLabel>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {STATUS.map((s, i) => (
              ...
              >
                ...
                <h3 className="font-mono text-sm font-semibold text-fd-foreground">{s.area}</h3>
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-6 py-12 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex flex-col gap-3">
          ...
          <p className="max-w-xs text-xs leading-relaxed text-fd-muted-foreground">
            A pure-Rust, Apache-2.0 reimplementation of PyMuPDF.
          </p>
        </div>

        <div className="grid grid-cols-2 gap-x-12 gap-y-2 text-sm sm:grid-cols-3">
          <FooterLink href="/docs">Docs</FooterLink>
          <FooterLink href="/docs/guide">Guide</FooterLink>
          <FooterLink href="/docs/reference">API reference</FooterLink>
          <FooterLink href="/docs/benchmarks">Benchmarks</FooterLink>
          <FooterLink href={githubUrl} external>
            GitHub
          </FooterLink>
          <FooterLink href={releasesUrl} external>
            Releases
          </FooterLink>
          <FooterLink href={githubUrl} external>
            License (Apache-2.0)
