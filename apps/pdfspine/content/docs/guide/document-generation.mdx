...
| `links` | bool | `True` | Write clickable URI/internal-link annotations (0.8.0). |
| `toc` | bool | `True` | Write heading hierarchy to the PDF outline (0.8.0). |
...
### Links and outline (0.8.0)

```python
doc = pdfspine.markdown_to_pdf("# Title\n\n[Website](https://example.com)")
print(doc.get_toc())
print(doc[0].get_links())
```

Both passes are enabled by default. Internal heading links resolve to matching
heading anchors; unresolved anchors get no annotation. Heading level jumps are
normalized for the outline. Use `links=False, toc=False` to disable both passes.
This does not download remote images or follow links.
