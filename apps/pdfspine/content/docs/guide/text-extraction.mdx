...
|---|---|---|
| `"layout"` | `str` | Visual lines with column spacing (pdfspine extension, 0.8.0). |
...
## Layout text and PDF → Markdown (0.8.0)

These are pdfspine extensions, separate from the PyMuPDF compatibility surface:

```python
layout = page.get_text("layout")
layout = page.get_text_layout(y_tolerance=3.0, char_width=None)
markdown = page.to_markdown()
markdown = doc.to_markdown(pages=[0])  # zero-based selection, in the given order
# Writes UTF-8 Markdown for all pages:
doc.save_markdown("document.md")
```

Layout output groups words around a line anchor and pads a character grid with
spaces. `char_width=None` uses the median glyph width. It preserves visual spacing,
not a semantic table model. Markdown uses font-size heading heuristics, lists,
emphasis and detected GFM tables; optional `images=True` adds placeholders.
It consumes native PDF text, not OCR. Inspect output for your documents.
This is the reverse direction of [`markdown_to_pdf`](/docs/guide/document-generation).
For model-assisted regions and semantic HTML, see [layout and tables](/docs/guide/layout-and-tables).

## Export a complete HTML document

`Page.get_text("html")` returns the reconstruction for one page. To combine
every page into a complete HTML5 document that can be opened directly in a
browser, use the document-level API:

```python
html = doc.to_html()

# Writes the same complete document as UTF-8; pathlib.Path also works.
doc.save_html("document.html")
```

The `<title>` uses the PDF metadata title when present, then falls back to the
input filename, and finally to `PDF document`. Title text is safely escaped.
Page fragments are preserved in page order inside the document body.

These APIs serve different scopes:

- `Page.get_text("html")` returns one page's HTML fragment.
- `Document.to_html()` returns a complete HTML5 document containing all pages.
- `Table.to_html()` returns only one detected table as an HTML `<table>`.
...
`"text"`, plus tuning knobs. The opt-in `"vision"` strategy is described in
