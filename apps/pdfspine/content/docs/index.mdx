...
pdfspine is **pre-1.0** (version `0.8.0`, published on PyPI —
...
The versioned reference for this update is [pdfspine 0.8.0](https://pypi.org/project/pdfspine/0.8.0/),
including its [compatibility matrix](https://github.com/VoldemortGin/pdfspine/blob/f1f6ab4208876b0ba867edd76cc4e5da7ad8add2/COMPAT.toml). Later `main` features are not part of this release baseline.
...
- **PDF → Markdown and layout text** via `to_markdown()`, `save_markdown()` and
  `get_text("layout")`; see [text extraction](/docs/guide/text-extraction).
- **Optional ONNX layout and tables**, with separately supplied weights; see
  [layout and tables](/docs/guide/layout-and-tables).
- **Optional-content layers** affect extraction and rendering; see
  [optional content](/docs/guide/optional-content).
