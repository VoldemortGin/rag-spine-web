...
Embedded pictures are **not rendered yet**: the export emits a `UserWarning` and
continues without the image (the picture's bytes stay available at parse level
via `image_bytes()`).
